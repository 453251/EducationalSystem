...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"AdminServlet?method=userList",
	        idField:'account', 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},		       
 		        ...
 		        {field:'role',title:'用户类别',width:100,
 		        	formatter:function(value,row,index)
 		        	{
 		        		// 根据当前域的值，返回是学生，教师还是管理员
 		        		if(value == 0)
 		        			{
 		        				return '学生';
 		        			}
        	...
        	if(selectRow == null){
            	...
            	$.messager.confirm("消息提醒", "将删除与用户相关的所有数据，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "AdminServlet?method=deleteUser",
	  ...
	  //导出按钮事件
	    $("#export").click(function(){
	    	var url = "AdminServlet?method=exportUserList";
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "AdminServlet?method=addUser",
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "AdminServlet?method=editUser",
				...
				},
				{
					...
					handler:function(){
						...
						else if(selectRow.role==1)
							{
								role="教师";
							}
						else if(selectRow.role==2)
							{
								role="管理员";
							}
						$("#edit_role").textbox('setValue', role);
					}
				}
			],
	...
	<div id="addDialog" style="padding: 10px">
    	<form id="addForm" method="post">
	    	<table cellpadding="8" >
	    		<tr>
	    			<td>账号:</td>
	    			<td><input id="add_account" class="easyui-textbox" style="width: 200px; height: 30px;" type="text" name="account" data-options="required:true,missingMessage:'请输入账号'" /></td>
	    		</tr>
	    		<tr>
	    			<td>姓名:</td>
	    		...
	    		<tr>
	    			<td>用户类别:</td>
	    			<td><select id="add_role" class="easyui-combobox" data-options="editable: false, panelHeight: 50, width: 60, height: 30" name="role"><option value=0>学生</option><option value=1>教师</option><option value=2>管理员</option></select></td>
	    		</tr>
	    	</table>
	    </form>
	</div>	
	...
	<div id="editDialog" style="padding: 10px">
    	<form id="editForm" method="post">
	    	<table cellpadding="8" >
	    		<tr>
	    			<td>账号:</td>
	    			<td><input id="edit_account" class="easyui-textbox" style="width: 200px; height: 30px;" type="text" name="account" data-options="required:true,missingMessage:'请输入账号'" /></td>
	    		</tr>
	    		<tr>
	    			<td>姓名:</td>
	    		...
	    		<tr>
	    			<td>用户类别:</td>
	    			<td><select id="edit_role" class="easyui-combobox" data-options="editable: false, panelHeight: 50, width: 60, height: 30" name="role"><option value=0>学生</option><option value=1>教师</option><option value=2>管理员</option></select></td>
	    		</tr>
	    	</table>
	    </form>
	</div>	
